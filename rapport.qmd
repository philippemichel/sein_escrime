---
subtitle: "Rapport Statistique - V 3.0"
cache: true
---
...
```{r}
#| label: tab-avap
#| tbl-cap: "Comparaison avant/après les 20 séances d'escrime"
tabx <- NULL
nn <- c(seq(16, 22, 2), seq(26, 32, 2))
for (i in nn) {
  med1 <- round(quantile(tt[[i]], na.rm = TRUE), 2)
  mm1 <- paste0(med1[3], " [", med1[2], ";", med1[4], "]")
  med2 <- round(quantile(tt[[i + 1]], na.rm = TRUE), 2)
  mm2 <- paste0(med2[3], " [", med2[2], ";", med2[4], "]")
  zz <- wilcox.test(tt[[i]], tt[[i + 1]], paired = TRUE)
  ll <- c(mm1, mm2, beaup(zz$p.value))
  tabx <- rbind(tabx, ll)
}
tabx <- as_tibble(tabx)
tabx$Item <- nscore
tabx <- relocate(tabx, Item)
names(tabx) <- c("Item", "Avant", "Après", "p-value")
tabx |>
  kbl(booktabs = T) |>
  kable_styling(latex_options = c("HOLD_position"))
```

On compare ici non pas les médianes globales mais les différences avant/après, cas par cas. L'hypothèse $\mathcal{H}_0$ est donc que la différence avant/après soit nulle. On utilise le test de Wilcoxon pour séries appariées, plus sensible ce qui explique la différence apparente de résultat avec les tableaux @tbl-differencephy & @tbl-diffmental.
...
## Mens sana in corpore sano {.smaller}
